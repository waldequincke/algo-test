 ...
 *
 * Uses a single linear scan (no regex, no full parse) for O(n) performance.
 * Escape sequences are skipped to avoid false positives on string content.
 */
...
async function bootstrap(): Promise<void> {
  ...
  // preParsing runs before the content-type parser so we can read the raw stream,
  // reject deep payloads immediately, and feed the clean bytes back to Fastify.
  ...
  // Layer 2: DTO validation (field types, nested structure)
  app.useGlobalPipes(
    new ValidationPipe({
      transform: true,
      whitelist: true,
  ...
  // Map TreeProcessingException → 400 { "error": "..." }
  app.useGlobalFilters(new TreeExceptionFilter());

  // X-Runtime-Ms and X-Runtime-Nanoseconds headers on every response
  app.useGlobalInterceptors(new TimingInterceptor());

  // OpenAPI / Swagger — mirrors quarkus-smallrye-openapi
  const document = SwaggerModule.createDocument(
    app,
    new DocumentBuilder()
      .setTitle('Algo Test — Node.js')
      .setDescription('BFS level-order tree traversal benchmark endpoint')
      .setVersion('1.0.0')
      .build(),
  );
